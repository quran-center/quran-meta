/*! 
 * Quran Meta library 1.0.2
 *
 * Released under the MIT license
 */

(function (factory) {
    typeof define === 'function' && define.amd ? define(factory) :
    factory();
}((function () { 'use strict';

    Object.defineProperty(exports, "__esModule", { value: true });
    var tslib_1 = require("tslib");
    // Quran Meta
    var const_1 = tslib_1.__importDefault(require("./const"));
    exports.meta = const_1.default;
    //export default QuranMeta;
    //------------------ Sura Data ---------------------
    // [start, ayas, order, rukus, name,  type, page ]
    var qdata_surah_json_1 = tslib_1.__importDefault(require("./qdata-surah.json"));
    exports.Sura = qdata_surah_json_1.default;
    var SuraList = qdata_surah_json_1.default;
    //------------------ Juz Data ---------------------
    // [sura, aya]
    var Juz = [
        [0, 0],
        [1, 1],
        [2, 142],
        [2, 253],
        [3, 93],
        [4, 24],
        [4, 148],
        [5, 82],
        [6, 111],
        [7, 88],
        [8, 41],
        [9, 93],
        [11, 6],
        [12, 53],
        [15, 1],
        [17, 1],
        [18, 75],
        [21, 1],
        [23, 1],
        [25, 21],
        [27, 56],
        [29, 46],
        [33, 31],
        [36, 28],
        [39, 32],
        [41, 47],
        [46, 1],
        [51, 31],
        [58, 1],
        [67, 1],
        [78, 1],
        [115, 1],
    ];
    exports.Juz = Juz;
    //------------------ Hizb Data ---------------------
    var qdata_hizb_json_1 = tslib_1.__importDefault(require("./qdata-hizb.json"));
    exports.HizbQuarter = qdata_hizb_json_1.default;
    //------------------ Manzil Data ---------------------
    var Manzil = [
        // [sura, aya]
        [],
        [1, 1],
        [5, 1],
        [10, 1],
        [17, 1],
        [26, 1],
        [37, 1],
        [50, 1],
    ];
    exports.Manzil = Manzil;
    //------------------ Ruku Data ---------------------
    // export Ruku  from "~/js/qdata-ruku.json"
    var qdata_ruku_json_1 = tslib_1.__importDefault(require("./qdata-ruku.json"));
    exports.Ruku = qdata_ruku_json_1.default;
    // [sura, aya]
    //------------------ Page Data ---------------------
    // code to get starting pages for surahs
    // let res=[];
    // Page.reduce((s,c,p)=>{
    // 	while (c[0]>=s)
    // 		{res.push([s,c[0],s==c[0]?p:p-1]);s+=1;}
    // 	return s
    // },0)
    var qdata_page_json_1 = tslib_1.__importDefault(require("./qdata-page.json"));
    exports.Page = qdata_page_json_1.default;
    var PageList = qdata_page_json_1.default;
    // [sura, aya]
    // export Page from "~/js/qdata-page.json"
    //------------------ Sajda Data ---------------------
    var Sajda = [
        // [sura, aya, type]
        [7, 206, "recommended"],
        [13, 15, "recommended"],
        [16, 50, "recommended"],
        [17, 109, "recommended"],
        [19, 58, "recommended"],
        [22, 18, "recommended"],
        [22, 77, "recommended"],
        [25, 60, "recommended"],
        [27, 26, "recommended"],
        [32, 15, "obligatory"],
        [38, 24, "recommended"],
        [41, 38, "obligatory"],
        [53, 62, "obligatory"],
        [84, 21, "recommended"],
        [96, 19, "obligatory"],
    ];
    exports.Sajda = Sajda;
    //export default QuranMeta;
    var ayaStringSplitter_1 = tslib_1.__importDefault(require("./ayaStringSplitter"));
    exports.ayaStringSplitter = ayaStringSplitter_1.default;
    /**
     *
     * @param {*} suraNumber
     * @param {*} ayaNumber
     */
    function findJuz(suraNumber, ayaNumber) {
        if (ayaNumber === void 0) { ayaNumber = 1; }
        var l = 1;
        while (Juz[l + 1][0] < suraNumber ||
            (Juz[l + 1][0] == suraNumber && Juz[l + 1][1] <= ayaNumber)) {
            l++;
        }
        return l;
    }
    exports.findJuz = findJuz;
    /**
     * Returns Positive number if aya is first ayah of juz, number is juz number
     * @param {*} ayaNumber
     * @param {*} suraNumber
     */
    function isAyahJuzFirst(suraNumber, ayaNumber) {
        return Juz.findIndex(function (x) { return x[0] == suraNumber && x[1] == ayaNumber; });
    }
    exports.isAyahJuzFirst = isAyahJuzFirst;
    //todo explain function
    /**
     * juz number
     * @param {*} suraNumber
     * @param {*} ayaNumber
     */
    function findJuzMetaBySurah(suraNumber, ayaNumber) {
        if (ayaNumber === void 0) { ayaNumber = 1; }
        var l = findJuz(suraNumber, ayaNumber);
        var r = l;
        while (Juz[r + 1][0] == suraNumber)
            r++;
        // console.log(l,r,
        //   "sura at start of file ",Juz[l][0],
        //   Sura[Juz[l][0]][0],
        //   Sura[suraNumber][0],
        //   Juz[l][1]
        // )
        var al = SuraList[suraNumber][0] - SuraList[Juz[l][0]][0] - Juz[l][1];
        // console.log(Sura[suraNumber + 1][0], Sura[Juz[l][0]][0])
        return [l, al + 1, r, getAyaCountinSura(suraNumber)];
    }
    exports.findJuzMetaBySurah = findJuzMetaBySurah;
    /**
     *
     * @param {*} suraNumber
     * @param {*} ayaNumber
     */
    function findPage(suraNumber, ayaNumber) {
        return (PageList.findIndex(function (x) {
            return (x[0] == suraNumber && x[1] > ayaNumber && x[1] > 1) ||
                x[0] > suraNumber;
        }) - 1);
    }
    exports.findPage = findPage;
    /**
     *
     * @param {*} ayaId
     */
    function findSurahByAyaid(ayaId) {
        var suraNum = SuraList.slice(1).findIndex(function (x) { return x[0] >= ayaId; });
        return suraNum < 0
            ? [114, ayaId - SuraList[114][0]]
            : [suraNum, ayaId - SuraList[suraNum][0]];
    }
    exports.findSurahByAyaid = findSurahByAyaid;
    /**
     *
     * @param {*} ayaId
     */
    function findJuzByAyaid(ayaId) {
        var _a = findSurahByAyaid(ayaId), s = _a[0], a = _a[1];
        return findJuz(s, a);
    }
    exports.findJuzByAyaid = findJuzByAyaid;
    /**
     *
     * @param {*} surah
     * @param {*} ayah
     */
    function findAyaidBySurah(surah, ayah) {
        var curSurahMeta = SuraList[surah];
        return curSurahMeta[0] + ayah;
    }
    exports.findAyaidBySurah = findAyaidBySurah;
    /**
     *
     * @param {*} surah
     */
    function getAyaCountinSura(surah) {
        return SuraList[surah][1];
    }
    exports.getAyaCountinSura = getAyaCountinSura;
    /**
     *
     * @param {*} surah
     * @param {*} ayah
     */
    function nextAyah(surah, ayah) {
        var ayaid = findAyaidBySurah(surah, ayah);
        return findSurahByAyaid(ayaid == const_1.default.numAyas ? 1 : ayaid + 1);
    }
    exports.nextAyah = nextAyah;
    /**
     *
     * @param {*} surah
     * @param {*} ayah
     */
    function prevAyah(surah, ayah) {
        var ayaid = findAyaidBySurah(surah, ayah);
        return findSurahByAyaid(ayaid == 1 ? const_1.default.numAyas : ayaid - 1);
    }
    exports.prevAyah = prevAyah;
    /**
     * Get the meta, first and last ayahs of the page
     * @param {*} pageNum
     */
    function pageMeta(pageNum) {
        var _a = [PageList[pageNum], PageList[pageNum + 1]], curPage = _a[0], nextPage = _a[1];
        return {
            pageNum: pageNum,
            first: curPage,
            last: tslib_1.__spreadArrays(findSurahByAyaid(findAyaidBySurah.apply(void 0, nextPage) - 1)),
        };
    }
    exports.pageMeta = pageMeta;
    /**
     * ALternative deprecated method
     * @param {*} pageNum
     */
    function pageMetaOld(pageNum) {
        var _a = [PageList[pageNum], PageList[pageNum + 1]], curPage = _a[0], nextPage = _a[1];
        var _b = [
            curPage[0],
            curPage[1],
            nextPage[1] === 1 ? nextPage[0] - 1 : nextPage[0],
            nextPage[1] === 1 ? SuraList[nextPage[0] - 1][1] : nextPage[1] - 1,
        ], firstSurah = _b[0], firstAyah = _b[1], lastSurah = _b[2], lastAyah = _b[3];
        return {
            pageNum: pageNum,
            first: [firstSurah, firstAyah],
            last: [lastSurah, lastAyah],
        };
    }
    exports.pageMetaOld = pageMetaOld;
    /**
     * Find range containing ayah according to the mode
     * @param {*} ayahId
     * @param {*} mode can be either 'all', 'juz', 'surah', 'ayah', 'page'
     * default is all
     */
    function findRangeAroundAyah(surah, ayah, mode) {
        switch (mode) {
            case "juz": {
                var juz = findJuz(surah, ayah);
                return [
                    findAyaidBySurah.apply(void 0, Juz[juz]),
                    findAyaidBySurah.apply(void 0, Juz[juz + 1]) - 1,
                ];
            }
            case "surah": {
                return [findAyaidBySurah(surah, 1), findAyaidBySurah(surah + 1, 1) - 1];
            }
            case "ayah": {
                var ayahId = findAyaidBySurah(surah, ayah);
                return [ayahId, ayahId];
            }
            case "page": {
                var page = findPage(surah, ayah);
                return [
                    findAyaidBySurah.apply(void 0, PageList[page]),
                    findAyaidBySurah.apply(void 0, PageList[page + 1]) - 1,
                ];
            }
            case "all":
            default:
                return [1, const_1.default.numAyas];
        }
    }
    exports.findRangeAroundAyah = findRangeAroundAyah;

})));
