...
export function checkKFQCQalun() {
  ...
  if (ayahCount !== meta.numAyahs) {
    console.log(`Error: ayah count is not matching: `, ayahCount, meta.numAyahs)
  }
  ...
  for (let ayahId: AyahId = 1; ayahId <= ayahCount; ayahId++) {
    ...
    if (ayahId !== qalMeta.id) {
      console.warn(`Error: ayahId of Ayah ${ayahId} are not matching: `, ayahMeta, qalMeta)
    }
