...
export function checkKFQCWarsh() {
  ...
  for (let ayahId: AyahId = 1; ayahId <= ayahCount; ayahId++) {
    ...
    if (ayahMeta.page !== Number.parseInt(warshMeta.page)) console.log(`Error: page of Ayah ${ayahId} are not matching: `, ayahMeta, warshMeta)
    ...
    if (ayahMeta.surah !== warshMeta.sura_no) console.warn(`Error: surah of Ayah ${ayahId} are not matching: `, ayahMeta, warshMeta)
    if (ayahMeta.ayah !== warshMeta.aya_no) console.warn(`Error: ayah of Ayah ${ayahId} are not matching: `, ayahMeta, warshMeta)
    if (ayahMeta.isSajdahAyah !== warshMeta.aya_text.includes("۩")) console.warn(`Error: sajdah of Ayah ${ayahId} are not matching: `, ayahMeta, warshMeta)
    if (ayahMeta.isStartOfQuarter !== warshMeta.aya_text.includes("۞") && ayahId !== 1) console.warn(`Error: RubAlHizb of Ayah ${ayahId} are not matching: `, ayahMeta, warshMeta)
    if (ayahMeta.ayah === 1 && warshMeta.sura_name_ar.trim() !== quran.getSurahMeta(ayahMeta.surah).name.trim()) console.warn(`Error: name of Surah of Ayah ${ayahId} are not matching: `, quran.getSurahMeta(ayahMeta.surah).name, warshMeta)
  }
}
/*
