...
import type { AyahNo, Juz, Surah, SurahInfo } from "../../../src/types"
...
export function checkQuranCloud() {
  ...
  for (let surahNo: Surah = 1; surahNo <= meta.numSurahs; surahNo++) {
    ...
    if (ayahCount !== sura.numberOfAyahs) {
      console.warn("Error QuranCloud surah: ", ayahCount, sura)
    }
    if (surahNo !== sura.number) {
      console.warn("Error QuranCloud surah: ", surahNo, sura)
    }
  ...
  for (let juzNo: Juz = 1; juzNo <= meta.numJuzs; juzNo++) {
    ...
    if (juzNo !== quran.findJuz(surah, ayah)) {
      console.warn("Error QuranCloud juz: ", ayahId, surah, ayah)
    }
  ...
  for (let rubHizb: Juz = 1; rubHizb <= meta.numRubAlHizbs; rubHizb++) {
    ...
    if (rubHizb !== quran.findRubAlHizb(surah, ayah)) {
      console.warn("Error QuranCloud rubAlHizb: ", ayahId, surah, ayah)
    }
    if (ayahId !== quran.findAyahIdBySurah(surah, ayah)) {
  ...
  for (let manzil = 1; manzil <= meta.numManzils; manzil++) {
    ...
    if (ayahId !== quran.findAyahIdBySurah(surah, ayah)) {
      console.warn("Error QuranCloud manzil: ", ayahId, surah, ayah)
    }
  }
  ...
  for (let rukuNo = 1; rukuNo <= meta.numRukus; rukuNo++) {
    ...
    if (ayahId !== quran.findAyahIdBySurah(surah, ayah)) {
      console.warn("Error QuranCloud ruku ", ayahId, surah, ayah)
    }
  }
  ...
  for (let page = 1; page <= meta.numPages; page++) {
    ...
    if (ayahId !== quran.findAyahIdBySurah(surah, ayah)) {
      console.warn("Error QuranCloud page: ", ayahId, surah, ayah)
    }
  }
  ...
  for (let sajda = 0; sajda < meta.numSajdas; sajda++) {
    ...
    if (ayahId !== quran.findAyahIdBySurah(surah, ayah)) {
      console.warn("Error QuranCloud sajda: ", ayahId, surah, ayah)
    }
  }
}
