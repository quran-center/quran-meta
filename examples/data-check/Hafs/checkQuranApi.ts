...
export function checkQuranApi() {
  ...
  for (let juzNo: Juz = 1; juzNo <= meta.numJuzs; juzNo++) {
    ...
    const { juz, start, end } = quranApi.juzs.references[juzNo - 1] as {
      juz: Juz
  ...
  for (let manzilNo: Manzil = 1; manzilNo <= meta.numManzils; manzilNo++) {
    ...
    const { manzil, start, end } = quranApi.manzils.references[manzilNo - 1] as {
      manzil: Manzil
      start: { chapter: Surah; verse: AyahNo }
      end: { chapter: Surah; verse: AyahNo }
    }
  ...
  for (let rukuNo: Ruku = 1; rukuNo <= meta.numRukus; rukuNo++) {
    ...
    const { ruku, start, end } = quranApi.rukus.references[rukuNo - 1] as {
      ruku: Ruku
      start: { chapter: Surah; verse: AyahNo }
      end: { chapter: Surah; verse: AyahNo }
    }
  ...
  for (let pageNo: Page = 1; pageNo <= meta.numPages; pageNo++) {
    ...
    const { page, start, end } = quranApi.pages.references[pageNo - 1] as {
      page: Page
      start: { chapter: Surah; verse: AyahNo }
      end: { chapter: Surah; verse: AyahNo }
    }
  ...
  for (let sajdaId = 0; sajdaId < meta.numSajdas; sajdaId++) {
    ...
    const {
      ...
      obligatory: _isObligatory
    } = quranApi.sajdas.references[sajdaId] as {
      sajda: number
      chapter: Surah
      verse: AyahNo
      recommended: boolean
