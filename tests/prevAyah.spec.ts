...
describe("prevAyah", () => {
  ...
  it("should return the previous ayah within the same surah", () => {
    expect(prevAyah(1, 2, HafsLists)).toEqual(expect.arrayContaining([1, 1]))
    expect(prevAyah(2, 286, HafsLists)).toEqual(expect.arrayContaining([2, 285]))
  })

  it("should return the last ayah of the previous surah when at the beginning of a surah", () => {
    expect(prevAyah(2, 1, HafsLists)).toEqual(expect.arrayContaining([1, 7]))
    expect(prevAyah(114, 1, HafsLists)).toEqual(expect.arrayContaining([113, 5]))
  })

  it("should wrap around to the last ayah of the last surah when at the beginning of the Quran", () => {
    expect(prevAyah(1, 1, HafsLists)).toEqual(expect.arrayContaining([114, 6]))
  })
  ...
  it("should handle edge cases", () => {
    expect(prevAyah(9, 128, HafsLists)).toEqual(expect.arrayContaining([9, 127]))
    expect(prevAyah(3, 1, HafsLists)).toEqual(expect.arrayContaining([2, 286]))
  })
})
