...
/**
 * Qunbul (from Ibn Kathir) entry point.
 *
 * The functional API with the Qunbul data already bound, so there is no `lists`
 * argument to pass. Everything is tree-shakeable.
 *
 * @example
 * ```typescript
 * import { meta, findJuz, getAyahMeta, quran } from "quran-meta/qunbul"
 *
 * meta.numAyahs // 6221
 * findJuz(2, 142)
 * getAyahMeta(1)
...
export { QunbulLists } from "./lists/QunbulLists"

/** Qunbul metadata: number of ayahs, pages, juzs, ... */
export const meta: QuranMeta = QunbulLists.meta

export const HizbQuarterList: AyahId[] = QunbulLists.HizbQuarterList
export const JuzList: AyahId[] = QunbulLists.JuzList
export const ManzilList: AyahId[] = QunbulLists.ManzilList
export const PageList: AyahId[] = QunbulLists.PageList
export const RukuList: AyahId[] = QunbulLists.RukuList
export const SajdaList: AyahId[] = QunbulLists.SajdaList
export const SurahList: SurahListType = QunbulLists.SurahList

// ==================== Lists and parsing ====================

/** Returns the raw boundary list for a kind of part */
export function getList(listName: PartType): AyahId[] | SurahListType {
  return _getList(listName, QunbulLists)
}

/** Returns the parts of a kind as `{ startAyahId, ayahCount }` blocks */
export function getListNormalised(listName: PartType): PartBlock[] {
  return _getListNormalised(listName, QunbulLists)
}
...
export function generatePartBlocks(part: PartType): PartBlock[] | null {
  return _generatePartBlocks(part, QunbulLists)
}
...
export function ayahStringSplitter(str: string, isStrict = true): SurahAyahSegment {
  return _ayahStringSplitter(str, isStrict, QunbulLists)
}
...
export function formatAyahId(ayahId: AyahId): string {
  return _formatAyahId(ayahId, QunbulLists)
}
...
export function checkValidSurahAyah(surah: unknown, ayah: unknown): asserts ayah is AyahNo {
  _checkValidSurahAyah(surah, ayah, QunbulLists)
}
...
export function checkValidSurahAyahPair(pair: [unknown, unknown]): asserts pair is SurahAyah {
  _checkValidSurahAyahPair(pair, QunbulLists)
}
...
export function isValidSurahAyah(x: [unknown, unknown]): x is SurahAyah {
  return _isValidSurahAyah(x, QunbulLists)
}
...
export function isValidRubAlHizb(x: unknown): x is RubAlHizbId {
  return _isValidRubAlHizb(x, QunbulLists)
}
...
export function getSurahMeta(surah: Surah): SurahMeta {
  return _getSurahMeta(surah, QunbulLists)
}
...
export function getPageMeta(page: Page): PageMeta {
  return _getPageMeta(page, QunbulLists)
}
...
export function getRubAlHizbMetaByAyahId(ayahId: AyahId): RubAlHizbMeta {
  return _getRubAlHizbMetaByAyahId(ayahId, QunbulLists)
}
...
export function findRangeAroundAyah(ayahId: AyahId, mode: RangeMode): AyahRange {
  return _findRangeAroundAyah(ayahId, mode, QunbulLists)
}
...
export function findRangeAroundSurahAyah(surah: Surah, ayah: AyahNo, mode: RangeMode): AyahRange {
  return _findRangeAroundSurahAyah(surah, ayah, mode, QunbulLists)
}
...
export function getPartRange(type: PartType, num: number): AyahRange {
  return _getPartRange(type, num, QunbulLists)
}
...
export function isAyahJuzFirst(ayahId: AyahId): number {
  return _isAyahJuzFirst(ayahId, QunbulLists)
}
...
export function isAyahPageFirst(ayahId: AyahId): number {
  return _isAyahPageFirst(ayahId, QunbulLists)
}
...
export function isSurahAyahJuzFirst(surah: Surah, ayah: AyahNo): number {
  return _isSurahAyahJuzFirst(surah, ayah, QunbulLists)
}
...
export function isSurahAyahPageFirst(surah: Surah, ayah: AyahNo): number {
  return _isSurahAyahPageFirst(surah, ayah, QunbulLists)
}
...
export function nextAyah(surah: Surah, ayah: AyahNo, options: AyahStepOptions = {}): SurahAyah | undefined {
  return _nextAyah(surah, ayah, QunbulLists, options)
}
...
export function prevAyah(surah: Surah, ayah: AyahNo, options: AyahStepOptions = {}): SurahAyah | undefined {
  return _prevAyah(surah, ayah, QunbulLists, options)
}

/** Converts a Qunbul `surah:ayah` to the numbering of another riwaya */
export function convertAyahTo(surah: Surah, ayah: AyahNo, to: RiwayaName): SurahAyah {
  return _convertAyah(surah, ayah, "Qunbul", to)
}

/** Converts a Qunbul `surah:ayah` to every ayah of another riwaya that shares its text */
export function convertAyahSpanTo(surah: Surah, ayah: AyahNo, to: RiwayaName): SurahAyahSegment {
  return _convertAyahSpan(surah, ayah, "Qunbul", to)
}

// ==================== Class-based API ====================

/**
 * Creates a QuranRiwaya instance for Qunbul
 */
export function createQunbul(): QuranRiwaya<"Qunbul"> {
  return QuranRiwaya.create<"Qunbul">(QunbulLists)
}

/**
 * Pre-initialized QuranRiwaya instance for Qunbul
 *
 * @example
 * ```typescript
 * import { quran } from "quran-meta/qunbul"
 *
 * quran.getAyahMeta(1)
 * quran.findJuz(2, 142)
 * ```
 */
export const quran: QuranRiwaya<"Qunbul"> = /* @__PURE__ */ createQunbul()
