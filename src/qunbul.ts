/**
 * Qunbul-specific entry point
 *
 * This module provides metadata and functional API for Qunbul riwaya only.
 * Functions are pre-bound to riwayaLists for convenience and optimal tree-shaking.
 *
 * @example
 * ```typescript
 * import { meta, findJuz, getAyahMeta } from 'quran-meta/qunbul'
 *
 * console.log(meta.numAyahs)  // 6221
 * const juz = findJuz(2, 142)
 * const ayahMeta = getAyahMeta(1)
 * ```
 */

import { QunbulLists as riwayaLists } from "./lists/QunbulLists"
import {
...
/**
 * Qunbul Lists (SurahList, JuzList, etc.)
 */
...
/**
 * Create a QuranRiwaya instance for Qunbul
 *
 * @example
 * ```typescript
 * import { createQunbul } from 'quran-meta/qunbul'
 *
 * const quran = createQunbul()
 * const ayahMeta = quran.getAyahMeta(1)
 * const juz = quran.findJuz(1, 1)
 * ```
 */
export const createQunbul = () => QuranRiwaya.create(riwayaLists)
