/**
 * Shuba-specific entry point
 *
 * This module provides metadata and functional API for Shuba riwaya only.
 * Functions are pre-bound to riwayaLists for convenience and optimal tree-shaking.
 *
 * @example
 * ```typescript
 * import { meta, findJuz, getAyahMeta } from 'quran-meta/shuba'
 *
...
import { ShubaLists as riwayaLists } from "./lists/ShubaLists"
import {
...
/**
 * Shuba Lists (SurahList, JuzList, etc.)
 */
...
/**
 * Create a QuranRiwaya instance for Shuba
 *
 * @example
 * ```typescript
 * import { createShuba } from 'quran-meta/shuba'
 *
 * const quran = createShuba()
 * const ayahMeta = quran.getAyahMeta(1)
 * const juz = quran.findJuz(1, 1)
 * ```
 */
export const createShuba = () => QuranRiwaya.create(riwayaLists)
