...
/**
 * Warsh (from Nafi') entry point.
 *
 * The functional API with the Warsh data already bound, so there is no `lists`
 * argument to pass. Everything is tree-shakeable.
 *
 * @example
 * ```typescript
 * import { meta, findJuz, getAyahMeta, quran } from "quran-meta/warsh"
 *
 * meta.numAyahs // 6214
 * findJuz(2, 142)
 * getAyahMeta(1)
...
export { WarshLists } from "./lists/WarshLists"

/** Warsh metadata: number of ayahs, pages, juzs, ... */
export const meta: QuranMeta = WarshLists.meta

export const HizbQuarterList: AyahId[] = WarshLists.HizbQuarterList
export const JuzList: AyahId[] = WarshLists.JuzList
export const ManzilList: AyahId[] = WarshLists.ManzilList
export const PageList: AyahId[] = WarshLists.PageList
export const RukuList: AyahId[] = WarshLists.RukuList
export const SajdaList: AyahId[] = WarshLists.SajdaList
export const SurahList: SurahListType = WarshLists.SurahList

// ==================== Lists and parsing ====================

/** Returns the raw boundary list for a kind of part */
export function getList(listName: PartType): AyahId[] | SurahListType {
  return _getList(listName, WarshLists)
}

/** Returns the parts of a kind as `{ startAyahId, ayahCount }` blocks */
export function getListNormalised(listName: PartType): PartBlock[] {
  return _getListNormalised(listName, WarshLists)
}
...
export function generatePartBlocks(part: PartType): PartBlock[] | null {
  return _generatePartBlocks(part, WarshLists)
}
...
export function ayahStringSplitter(str: string, isStrict = true): SurahAyahSegment {
  return _ayahStringSplitter(str, isStrict, WarshLists)
}
...
export function formatAyahId(ayahId: AyahId): string {
  return _formatAyahId(ayahId, WarshLists)
}
...
export function checkValidSurahAyah(surah: unknown, ayah: unknown): asserts ayah is AyahNo {
  _checkValidSurahAyah(surah, ayah, WarshLists)
}
...
export function checkValidSurahAyahPair(pair: [unknown, unknown]): asserts pair is SurahAyah {
  _checkValidSurahAyahPair(pair, WarshLists)
}
...
export function isValidSurahAyah(x: [unknown, unknown]): x is SurahAyah {
  return _isValidSurahAyah(x, WarshLists)
}
...
export function isValidRubAlHizb(x: unknown): x is RubAlHizbId {
  return _isValidRubAlHizb(x, WarshLists)
}
...
export function getSurahMeta(surah: Surah): SurahMeta {
  return _getSurahMeta(surah, WarshLists)
}
...
export function getRubAlHizbMetaByAyahId(ayahId: AyahId): RubAlHizbMeta {
  return _getRubAlHizbMetaByAyahId(ayahId, WarshLists)
}
...
export function findRangeAroundAyah(ayahId: AyahId, mode: RangeMode): AyahRange {
  return _findRangeAroundAyah(ayahId, mode, WarshLists)
}
...
export function findRangeAroundSurahAyah(surah: Surah, ayah: AyahNo, mode: RangeMode): AyahRange {
  return _findRangeAroundSurahAyah(surah, ayah, mode, WarshLists)
}
...
export function getPartRange(type: PartType, num: number): AyahRange {
  return _getPartRange(type, num, WarshLists)
}
...
export function isAyahJuzFirst(ayahId: AyahId): number {
  return _isAyahJuzFirst(ayahId, WarshLists)
}
...
export function isAyahPageFirst(ayahId: AyahId): number {
  return _isAyahPageFirst(ayahId, WarshLists)
}
...
export function isSurahAyahJuzFirst(surah: Surah, ayah: AyahNo): number {
  return _isSurahAyahJuzFirst(surah, ayah, WarshLists)
}
...
export function isSurahAyahPageFirst(surah: Surah, ayah: AyahNo): number {
  return _isSurahAyahPageFirst(surah, ayah, WarshLists)
}
...
export function nextAyah(surah: Surah, ayah: AyahNo, options: AyahStepOptions = {}): SurahAyah | undefined {
  return _nextAyah(surah, ayah, WarshLists, options)
}
...
export function prevAyah(surah: Surah, ayah: AyahNo, options: AyahStepOptions = {}): SurahAyah | undefined {
  return _prevAyah(surah, ayah, WarshLists, options)
}

/** Converts a Warsh `surah:ayah` to the numbering of another riwaya */
export function convertAyahTo(surah: Surah, ayah: AyahNo, to: RiwayaName): SurahAyah {
  return _convertAyah(surah, ayah, "Warsh", to)
}

/** Converts a Warsh `surah:ayah` to every ayah of another riwaya that shares its text */
export function convertAyahSpanTo(surah: Surah, ayah: AyahNo, to: RiwayaName): SurahAyahSegment {
  return _convertAyahSpan(surah, ayah, "Warsh", to)
}

// ==================== Class-based API ====================

/**
 * Creates a QuranRiwaya instance for Warsh
 */
export function createWarsh(): QuranRiwaya<"Warsh"> {
  return QuranRiwaya.create<"Warsh">(WarshLists)
}
 ...
 * ```
 */
export const quran: QuranRiwaya<"Warsh"> = /* @__PURE__ */ createWarsh()
