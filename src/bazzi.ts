...
/**
 * Bazzi (from Ibn Kathir) entry point.
 *
 * The functional API with the Bazzi data already bound, so there is no `lists`
 * argument to pass. Everything is tree-shakeable.
 *
 * @example
 * ```typescript
 * import { meta, findJuz, getAyahMeta, quran } from "quran-meta/bazzi"
 *
 * meta.numAyahs // 6221
 * findJuz(2, 142)
 * getAyahMeta(1)
...
export { BazziLists } from "./lists/BazziLists"

/** Bazzi metadata: number of ayahs, pages, juzs, ... */
export const meta: QuranMeta = BazziLists.meta
...
export function generatePartBlocks(part: PartType): PartBlock[] | null {
  return _generatePartBlocks(part, BazziLists)
}
...
export function ayahStringSplitter(str: string, isStrict = true): SurahAyahSegment {
  return _ayahStringSplitter(str, isStrict, BazziLists)
}
...
export function formatAyahId(ayahId: AyahId): string {
  return _formatAyahId(ayahId, BazziLists)
}
...
export function checkValidSurahAyah(surah: unknown, ayah: unknown): asserts ayah is AyahNo {
  _checkValidSurahAyah(surah, ayah, BazziLists)
}
...
export function checkValidSurahAyahPair(pair: [unknown, unknown]): asserts pair is SurahAyah {
  _checkValidSurahAyahPair(pair, BazziLists)
}
...
export function isValidSurahAyah(x: [unknown, unknown]): x is SurahAyah {
  return _isValidSurahAyah(x, BazziLists)
}
...
export function isValidRubAlHizb(x: unknown): x is RubAlHizbId {
  return _isValidRubAlHizb(x, BazziLists)
}
...
export function getSurahMeta(surah: Surah): SurahMeta {
  return _getSurahMeta(surah, BazziLists)
}
...
export function getRubAlHizbMetaByAyahId(ayahId: AyahId): RubAlHizbMeta {
  return _getRubAlHizbMetaByAyahId(ayahId, BazziLists)
}
...
export function findRangeAroundAyah(ayahId: AyahId, mode: RangeMode): AyahRange {
  return _findRangeAroundAyah(ayahId, mode, BazziLists)
}
...
export function findRangeAroundSurahAyah(surah: Surah, ayah: AyahNo, mode: RangeMode): AyahRange {
  return _findRangeAroundSurahAyah(surah, ayah, mode, BazziLists)
}
...
export function getPartRange(type: PartType, num: number): AyahRange {
  return _getPartRange(type, num, BazziLists)
}
...
export function isAyahJuzFirst(ayahId: AyahId): number {
  return _isAyahJuzFirst(ayahId, BazziLists)
}
...
export function isAyahPageFirst(ayahId: AyahId): number {
  return _isAyahPageFirst(ayahId, BazziLists)
}
...
export function isSurahAyahJuzFirst(surah: Surah, ayah: AyahNo): number {
  return _isSurahAyahJuzFirst(surah, ayah, BazziLists)
}
...
export function isSurahAyahPageFirst(surah: Surah, ayah: AyahNo): number {
  return _isSurahAyahPageFirst(surah, ayah, BazziLists)
}
...
export function nextAyah(surah: Surah, ayah: AyahNo, options: AyahStepOptions = {}): SurahAyah | undefined {
  return _nextAyah(surah, ayah, BazziLists, options)
}
...
export function prevAyah(surah: Surah, ayah: AyahNo, options: AyahStepOptions = {}): SurahAyah | undefined {
  return _prevAyah(surah, ayah, BazziLists, options)
}

/** Converts a Bazzi `surah:ayah` to the numbering of another riwaya */
export function convertAyahTo(surah: Surah, ayah: AyahNo, to: RiwayaName): SurahAyah {
  return _convertAyah(surah, ayah, "Bazzi", to)
}

/** Converts a Bazzi `surah:ayah` to every ayah of another riwaya that shares its text */
export function convertAyahSpanTo(surah: Surah, ayah: AyahNo, to: RiwayaName): SurahAyahSegment {
  return _convertAyahSpan(surah, ayah, "Bazzi", to)
}

// ==================== Class-based API ====================

/**
 * Creates a QuranRiwaya instance for Bazzi
 */
export function createBazzi(): QuranRiwaya<"Bazzi"> {
  return QuranRiwaya.create<"Bazzi">(BazziLists)
}

/**
 * Pre-initialized QuranRiwaya instance for Bazzi
 *
 * @example
 * ```typescript
 * import { quran } from "quran-meta/bazzi"
 *
 * quran.getAyahMeta(1)
 * quran.findJuz(2, 142)
 * ```
 */
export const quran: QuranRiwaya<"Bazzi"> = /* @__PURE__ */ createBazzi()
