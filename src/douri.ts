/**
 * Douri-specific entry point
 *
 * This module provides metadata and functional API for Douri riwaya only.
 * Functions are pre-bound to riwayaLists for convenience and optimal tree-shaking.
 *
 * @example
 * ```typescript
 * import { meta, findJuz, getAyahMeta } from 'quran-meta/douri'
 *
 * console.log(meta.numAyahs)  // 6217
 * const juz = findJuz(2, 142)
 * const ayahMeta = getAyahMeta(1)
 * ```
 */

import { DouriLists as riwayaLists } from "./lists/DouriLists"
import {
...
/**
 * Douri Lists (SurahList, JuzList, etc.)
 */
...
/**
 * Create a QuranRiwaya instance for Douri
 *
 * @example
 * ```typescript
 * import { createDouri } from 'quran-meta/douri'
 *
 * const quran = createDouri()
 * const ayahMeta = quran.getAyahMeta(1)
 * const juz = quran.findJuz(1, 1)
 * ```
 */
export const createDouri = () => QuranRiwaya.create(riwayaLists)
