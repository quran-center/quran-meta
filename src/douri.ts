...
/**
 * Douri (from Abu 'Amr) entry point.
 *
 * The functional API with the Douri data already bound, so there is no `lists`
 * argument to pass. Everything is tree-shakeable.
 *
 * @example
 * ```typescript
 * import { meta, findJuz, getAyahMeta, quran } from "quran-meta/douri"
 *
 * meta.numAyahs // 6217
 * findJuz(2, 142)
 * getAyahMeta(1)
...
export { DouriLists } from "./lists/DouriLists"

/** Douri metadata: number of ayahs, pages, juzs, ... */
export const meta: QuranMeta = DouriLists.meta
...
export function generatePartBlocks(part: PartType): PartBlock[] | null {
  return _generatePartBlocks(part, DouriLists)
}
...
export function ayahStringSplitter(str: string, isStrict = true): SurahAyahSegment {
  return _ayahStringSplitter(str, isStrict, DouriLists)
}
...
export function formatAyahId(ayahId: AyahId): string {
  return _formatAyahId(ayahId, DouriLists)
}
...
export function checkValidSurahAyah(surah: unknown, ayah: unknown): asserts ayah is AyahNo {
  _checkValidSurahAyah(surah, ayah, DouriLists)
}
...
export function checkValidSurahAyahPair(pair: [unknown, unknown]): asserts pair is SurahAyah {
  _checkValidSurahAyahPair(pair, DouriLists)
}
...
export function isValidSurahAyah(x: [unknown, unknown]): x is SurahAyah {
  return _isValidSurahAyah(x, DouriLists)
}
...
export function isValidRubAlHizb(x: unknown): x is RubAlHizbId {
  return _isValidRubAlHizb(x, DouriLists)
}
...
export function getSurahMeta(surah: Surah): SurahMeta {
  return _getSurahMeta(surah, DouriLists)
}
...
export function getPageMeta(page: Page): PageMeta {
  return _getPageMeta(page, DouriLists)
}
...
export function getRubAlHizbMetaByAyahId(ayahId: AyahId): RubAlHizbMeta {
  return _getRubAlHizbMetaByAyahId(ayahId, DouriLists)
}
...
export function findRangeAroundAyah(ayahId: AyahId, mode: RangeMode): AyahRange {
  return _findRangeAroundAyah(ayahId, mode, DouriLists)
}
...
export function findRangeAroundSurahAyah(surah: Surah, ayah: AyahNo, mode: RangeMode): AyahRange {
  return _findRangeAroundSurahAyah(surah, ayah, mode, DouriLists)
}
...
export function getPartRange(type: PartType, num: number): AyahRange {
  return _getPartRange(type, num, DouriLists)
}
...
export function isAyahJuzFirst(ayahId: AyahId): number {
  return _isAyahJuzFirst(ayahId, DouriLists)
}
...
export function isAyahPageFirst(ayahId: AyahId): number {
  return _isAyahPageFirst(ayahId, DouriLists)
}
...
export function isSurahAyahJuzFirst(surah: Surah, ayah: AyahNo): number {
  return _isSurahAyahJuzFirst(surah, ayah, DouriLists)
}
...
export function isSurahAyahPageFirst(surah: Surah, ayah: AyahNo): number {
  return _isSurahAyahPageFirst(surah, ayah, DouriLists)
}
...
export function nextAyah(surah: Surah, ayah: AyahNo, options: AyahStepOptions = {}): SurahAyah | undefined {
  return _nextAyah(surah, ayah, DouriLists, options)
}
...
export function prevAyah(surah: Surah, ayah: AyahNo, options: AyahStepOptions = {}): SurahAyah | undefined {
  return _prevAyah(surah, ayah, DouriLists, options)
}

/** Converts a Douri `surah:ayah` to the numbering of another riwaya */
export function convertAyahTo(surah: Surah, ayah: AyahNo, to: RiwayaName): SurahAyah {
  return _convertAyah(surah, ayah, "Douri", to)
}

/** Converts a Douri `surah:ayah` to every ayah of another riwaya that shares its text */
export function convertAyahSpanTo(surah: Surah, ayah: AyahNo, to: RiwayaName): SurahAyahSegment {
  return _convertAyahSpan(surah, ayah, "Douri", to)
}

// ==================== Class-based API ====================

/**
 * Creates a QuranRiwaya instance for Douri
 */
export function createDouri(): QuranRiwaya<"Douri"> {
  return QuranRiwaya.create<"Douri">(DouriLists)
}

/**
 * Pre-initialized QuranRiwaya instance for Douri
 *
 * @example
 * ```typescript
 * import { quran } from "quran-meta/douri"
 *
 * quran.getAyahMeta(1)
 * quran.findJuz(2, 142)
 * ```
 */
export const quran: QuranRiwaya<"Douri"> = /* @__PURE__ */ createDouri()
